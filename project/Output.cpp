...
Output::Output()
{
	...
	UI.PlayerColors[0] = ORANGERED;
	UI.PlayerColors[1] = CYAN;
	UI.PlayerColors[2] = YELLOW;
	UI.PlayerColors[3] = FORESTGREEN;

	// Create the output window
	pWind = CreateWind(UI.width + 15, UI.height, UI.wx, UI.wy);

	// Change the title
	pWind->ChangeTitle("Snakes & Ladders & Monopoly");
...
int Output::GetCellStartX(const CellPosition & cellPos) const
{
	...
	return ((UI.CellWidth*cellPos.HCell())); // this line should be changed with your implementation
}
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\ROLL_DICE.jpg";
	MenuItemImages[ITM_DICE_VALUE] = "images\\DICE_VALUE.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	...
	pWind->GetStringSize(w, h, info);	// Set the start X & Y coordinate of drawing the string
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	int cellwidth = UI.width / NumHorizontalCells;
	int cellheight = (UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / 9;
	...
	CellPosition v;
	int e = v.GetCellNumFromPosition(cellPos);
	pWind->GetIntegerSize(w, h, e);
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (toCell.VCell() < fromCell.VCell() && toCell.HCell() == fromCell.HCell())
	{
		...
		{
			pWind->DrawLine(x12, i*UI.CellHeight + UI.ToolBarHeight, x34, i*UI.CellHeight + UI.ToolBarHeight);
		}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell.VCell() < toCell.VCell() && fromCell.HCell() == toCell.HCell())
	{
		...
		int Y[4] = { y1 + yChange,(y1 + 2 * yChange),y1 + yChange,y1 };
